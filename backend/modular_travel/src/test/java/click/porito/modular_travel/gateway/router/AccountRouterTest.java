...
import click.porito.modular_travel.image.ImageInternalManagement;
...
class AccountRouterTest {
    ...
    class getDetailAccount {
        ...
        void getAccountDetailInfo() throws Exception {
            ...
            AccountDTO detailedProfile = AccountDTO.AccountDtoImpl.builder()
                    ...
                    .email("test@test.com")
                    .build();
            doReturn(detailedProfile).when(accountExternalApi).getCurrentAccount();
            ...
            //then
            verify(accountExternalApi, times(1)).getCurrentAccount();
        ...
        void getAccountDetailInfoWithInvalidAuthentication() throws Exception {
            //given
            doThrow(new InvalidAuthenticationException()).when(accountExternalApi).getCurrentAccount();
    ...
    class getDetailedAccountInfo{
        ...
        void getSimpleProfile() throws Exception {
            ...
            AccountSummaryDTO profile = Account.builder("email@test.com", Role.USER)
                    ...
                    .build();
            doReturn(profile).when(accountExternalApi).getCurrentAccountSummary();
            ...
            //then
            verify(accountExternalApi, times(1)).getCurrentAccountSummary();
            String content = mvcResult.getResponse().getContentAsString();
        ...
        void getSimpleProfileWithInvalidAuthentication() throws Exception {
            //given
            doThrow(new InvalidAuthenticationException()).when(accountExternalApi).getCurrentAccountSummary();
    ...
    class deleteAccount {
        ...
        void deleteAccount() throws Exception {
            //given
            doNothing().when(accountExternalApi).deleteCurrentAccount();
            //when
            mvc.perform(delete("/account"))
                    //then
                    .andExpect(status().isNoContent());
            //then
            verify(accountExternalApi, times(1)).deleteCurrentAccount();
        }
    ...
    class patchProfileInfo {
        ...
        void patchProfileInfoWithoutAuthentication() throws Exception {
            ...
            //then
            verify(accountExternalApi, times(0)).patchProfileInfo(any());
        ...
        void patchProfile() throws Exception {
            //given
            doNothing().when(accountExternalApi).patchProfileInfo(any());
            AccountPatchDTO patchRequest = new AccountPatchDTO();
            patchRequest.setBirthDate(LocalDate.of(1999, 1, 1));
            String json = objectMapper.writeValueAsString(patchRequest);
            //when
            ...
            //then
            verify(accountExternalApi, times(1)).patchProfileInfo(any());

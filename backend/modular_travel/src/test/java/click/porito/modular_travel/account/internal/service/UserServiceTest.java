...
class UserServiceTest {
    ...
    class getProfile {
        ...
        @Test
        @DisplayName("유저가 존재할 경우 SimpleProfile 반환")
        void getProfile2() {
            //given
            long userId = 1L;
            String name = "test";
            Account user = Account.builder("test@email.com", Account.Role.USER)
                    .name(name)
                    .gender(Account.Gender.MALE)
                    .build();
            when(accountRepository.findById(userId)).thenReturn(Optional.of(user));
            //when
            SimpleProfile response = userService.getProfile(userId);
            //then

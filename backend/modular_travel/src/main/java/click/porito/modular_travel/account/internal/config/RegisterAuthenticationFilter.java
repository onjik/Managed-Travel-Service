...
public class RegisterAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
    ...
    public RegisterAuthenticationFilter(ObjectMapper objectMapper, Validator validator, AccountService accountService) {
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException, IOException, ServletException {
        ...
        AccountRegisterForm registerForm = new AccountRegisterForm(exception.getRegisterForm()); //copy - for prevent side effect
        ...
        Account account = accountService.registerAccount(registerForm);
    ...
    protected AccountRegisterForm parseBody(HttpServletRequest request) {
        //check if body exists
        if (request.getContentLength() == 0){
            throw new AuthenticationCredentialsNotFoundException("invalid request");
        }
        try {
            return objectMapper.readValue(request.getInputStream(), AccountRegisterForm.class);
        } catch (Exception e){
            throw new AuthenticationCredentialsNotFoundException("invalid request");
        }
    }

...
public class PlaceOperationCacheAspect {
    ...
    @Around("execution(java.util.Optional<click.porito.managed_travel.place.domain.Place> click.porito.managed_travel.place.place_service.operation.application.PlaceOperation.getPlace(String)) && args(placeId)")
    ...
    @SuppressWarnings("unchecked")
    @Around("execution(java.util.List<click.porito.managed_travel.place.domain.Place> click.porito.managed_travel.place.place_service.operation.application.PlaceOperation.getPlaces(String[])) && args(placeIds)")
    public List<Place> getPlaces(ProceedingJoinPoint joinPoint, String[] placeIds) throws Throwable {
        ...
        if (matchedCache.size() == placeIds.length) {
            log.debug("Cache Hit - Get Places From Cache : {}", (Object) placeIds);
            return matchedCache;
        }
        ...
        if (!result.isEmpty()) {
            // cache
            log.debug("Cache Put - Put Places To Cache : {}", (Object) unmatchedPlaceIds);
    ...
    @SuppressWarnings("unchecked")
    @Around("execution(java.util.List<click.porito.managed_travel.place.domain.Place> click.porito.managed_travel.place.place_service.operation.application.PlaceOperation.getPlaceNearBy(double, double, int, java.lang.Integer, click.porito.managed_travel.place.domain.PlaceType[], java.lang.Boolean))")
    ...
    @Around("execution(boolean click.porito.managed_travel.place.place_service.operation.application.PlaceOperation.exists(String)) && args(placeId)")

    ...
    protected <T> Optional<T> exchange(String uri, HttpMethod httpMethod, HttpEntity<?> httpEntity, TypeToken<T> outputType){
        try {
            ...
            if (code == 403) {
                ...
                if (message.contains("Not a valid Place ID")) {
            ...
            //여기 까지 오면 에러
            throw new UnexpectedExternalApiException("Unexpected Error", e);
        }
        //여기 까지 오면 에러
        throw new UnexpectedExternalApiException("Unexpected Error", null);
    }
